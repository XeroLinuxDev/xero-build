...
**Issue 1 :** On the **KDE Flagship** there's a regression with *kwin_wayland* causing CPU to be pegged at near 100% during install for **nVidia** users. Please be patient with it, try to get it installed, once on your desktop make sure to update, then install GPU drivers n reboot. Issue should be gone. Nothing we can do except wait until issue is resolved. Sorry about that.
